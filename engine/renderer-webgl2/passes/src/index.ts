...
/** Parent-child transform solver for 2D hierarchies. */
...
/** Smoothly follows a world-space target with frame-rate independent lerp. */
...
/** Converts world coordinates to viewport pixel coordinates. */
...
/** Converts viewport pixel coordinates back into world space. */
...
/** CPU-side sprite batch builder with texture and blend bucketing. */
export class SpriteBatch {
  private readonly capacity: number
  private readonly commands: DrawSpriteInput[] = []
  ...
  constructor(maxSprites = 2048) {
    this.capacity = Math.max(1, Math.floor(maxSprites))
  ...
  flush(): BatchStats {
    const sorted = [...this.commands].sort((a, b) => {
      const az = a.transform.zIndex
      const bz = b.transform.zIndex
      if (az !== bz) {
        return az - bz
      }

      const at = getTextureKey(a.sprite.texture)
      const bt = getTextureKey(b.sprite.texture)
      if (at !== bt) {
        return at - bt
      }

      if (a.sprite.blendMode < b.sprite.blendMode) {
        return -1
      }
      if (a.sprite.blendMode > b.sprite.blendMode) {
        return 1
    ...
    for (const cmd of sorted) {
      const needsFlush =
        ...
        currentTexture !== cmd.sprite.texture ||
        currentBlend !== cmd.sprite.blendMode
...
/** Bitmap/SDF font metadata with atlas-backed glyph lookup. */
export class Font {
...
/** Lays out text with wrapping and alignment for glyph-quad rendering. */
...
/** Emits text glyphs as sprite batch draw commands. */
...
/** Debug/UI primitive builder for lines, quads, circles, and polygons. */
export class PrimitiveBatch {
  ...
  drawLine(start: Vec2, end: Vec2, color: Color, thickness = 1): void {
    ...
    // Thick line = oriented quad around segment centerline.
  ...
  drawPolygon(points: Vec2[], color: Color, filled: boolean): void {
    ...
    // Fan triangulation is O(n) and enough for convex debug shapes.
    const root = points[0]
    if (!root) {
      return
    }
...
/** Render target abstraction over texture-backed framebuffers. */
...
/** Dependency-sorted pass graph for multi-pass rendering pipelines. */
