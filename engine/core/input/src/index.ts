...
/** Cross-platform input state with per-frame edge tracking. */
export class InputManager {
  ...
  beginFrame(): void {
    // Kept for symmetry with endFrame-driven reset.
  }
  ...
  setGamepadAxisState(gamepad: number, axis: number, value: number): void {
    const state = this.getOrCreateGamepad(gamepad)
    // NaN axis values happen on flaky drivers. We choose "pretend centered" over chaos.
...
/** Action bindings layered over raw input state. */
